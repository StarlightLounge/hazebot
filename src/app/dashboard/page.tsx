...
export default function Dashboard() {
  ...
  // Mock servers for the dashboard UI
  const servers = [
    { id: 1, name: "The Chill Zone", icon: "/assets/logo.png", hasBot: true },
    { id: 2, name: "Stream Community", icon: null, hasBot: true },
    { id: 3, name: "Gamer Lounge", icon: null, hasBot: false },
  ];
        ...
        <div className="server-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '2rem' }}>
          {servers.map((server) => (
            <div key={server.id} className="glass-card" style={{ padding: '2rem', display: 'flex', alignItems: 'center', gap: '1.5rem', cursor: 'pointer', transition: 'var(--transition)' }} 
              ...
              <div style={{ flex: 1 }}>
                <h3 style={{ fontSize: '1.25rem', marginBottom: '0.25rem' }}>{server.name}</h3>
                {server.hasBot ? (
